<div class="container row" id="booking">
	<script>
		//<script>
		var todayD = Number('<?= date('d') ?>');
		var todayM = Number('<?= date('m') ?>');
		var todayY = Number('<?= date('Y') ?>');
		var todayFull = todayD + '/' + todayM + '/' + todayY;
		var tomorowFull = (todayD+1) + '/' + todayM + '/' + todayY;
		
        var data = '<div class="CloudBedsWidget  ver-"> \
      <div action="https://hotels.cloudbeds.com/en/reservas/bLvGyU" target="_blank" method="post" autocomplete="off" class="widgetHotelsForm"> \
        <div class="horizontal-widget "> \
        <input type="hidden" name="date_format" value="d/m/Y" /> \
        <div class="acessa_widget_block" id="checkin"><p title="<?php echo __('Check In', 'html5blank'); ?>"><?php echo __('Check In', 'html5blank'); ?>:</p> \
        <input type="text" class="widgetHotelsInputText date"  name="widget_date" value="'+todayFull+'"> \
        <input type="date" name="start_date_mobile" class="date_mobile widget_hide" id="start_date_mobile"></div>\
        <div class="acessa_widget_block" id="checkout"><p title="<?php echo __('Check Out', 'html5blank'); ?>"><?php echo __('Check Out', 'html5blank'); ?>:</p>\
        <input type="text" class="widgetHotelsInputText date"  name="widget_date_to" value="'+tomorowFull+'">\
        <input type="date" name="end_date_mobile" class="date_mobile widget_hide" id="end_date_mobile"></div> \
        <input type="button"  value="<?php echo __('Book Now', 'html5blank'); ?>" > \
        </div>\
      </div>\
    </div>';
//<script>
        ...
        var CloudBeds_widget = function () {
            ...
            function momentLoad() {
                if (typeof moment == 'undefined'){
                    LoadScript('https://hotels.cloudbeds.com/plugins/moment/moment.min.js', checkPikaday);
                } else {
                    checkPikaday();
                }
            }
            ...
            function _bind_dates() {
                if (typeof Pikaday === 'function') {
                    var startDate = new Date(todayY, todayM-1, todayD, 0, 0, 0, 0),
                        nextDay = new Date(todayY, todayM-1, todayD+1, 0, 0, 0, 0),
                        maxDate = new Date(todayY+2, todayM-1, todayD+1, 0, 0, 0, 0);
                    ...
                    var checkinPicker = new Pikaday({
                        ...
                        firstDay: 0,
                        isRTL: '',
                        container: document.getElementById('checkin'),
                        i18n: {
                            previousMonth: '&#x3c;Prev',
                            nextMonth: 'Next&#x3e;',
                        ...
                        onSelect: function () {
                            ...
                            checkoutPicker.setMaxDate(maxDate);
            ...
            function _bind_submit() {
                ...
                if (form) {
                    var _submit = document.querySelectorAll(baseClass+'[type="button"]')[0];
                    ...
                    function submitWidgetForm(event) {
                        ...
                        realForm.action = form.getAttribute('action');
                        realForm.method = form.getAttribute('method');
                        realForm.target = form.getAttribute('target');
                        realForm.class = 'widget_hide';
                        realForm.appendChild(form.cloneNode(true));
                        pageBody.appendChild(realForm);

                        if (ga_onde && ! __ofGa.q) {
                            __ofGa('ondeFicarTracker.set', 'dimension1', dimensionValue);

                            __ofGa('ondeFicarTracker.send', 'event', 'Reserva Widget', 'Buscar Click', 'horiz', {
                                'hitCallback': function () {
                                    var e = document.createElement('div');
                                    e.innerHTML = '<input type="hidden" name="ga_sess_id" value="' + ga_sess + '" />';
                                    realForm.appendChild(e);
                                    window.console && window.console.log('Cloudbeds Widget - Buscar Click');

                                    realForm.submit();
                                    pageBody.removeChild(realForm);
                                }
                            });
                        } else {
            ...
            if (ga_onde) {
                ...
                __ofGa('ondeFicarTracker.send', 'event', 'Reserva Widget', 'Widget Load', 'horiz', {
                    'nonInteraction': 1,
                    'hitCallback': function () {
